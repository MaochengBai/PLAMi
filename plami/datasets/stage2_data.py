...
class CustomDataset(Dataset):
    ...
    #     if not mask_ann:
    #         # 记录有问题的情况，但不抛出异常
    #         print("Warning: mask_ann is an empty list. Returning empty mask.")
    #         # 返回一个全零的掩码数组
    ...
    #         # 对于其他类型的错误，也可以选择返回一个空掩码，或者根据需要采取其他措施
    #         return np.zeros((h, w), dtype=np.uint8)
    #
    #     return mask
    ...
    def annToMask(self, mask_ann, h, w):
        ...
        # 检查mask_ann是否是列表
        if isinstance(mask_ann, list):
            # 遍历列表中的每个多边形（假设每个多边形是顶点的列表）
            for shape in mask_ann:
                # 将顶点列表转换为适用于cv2.fillPoly的numpy数组
                if isinstance(shape, list):  # 确保每个形状也是一个列表
                    # 确保顶点数组的形状是(n, 1, 2)
                    points = np.array([shape], dtype=np.int32)
                    points = points.reshape((-1, 1, 2))
                    # 使用cv2.fillPoly填充多边形，填充色为1
